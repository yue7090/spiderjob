...
import (
	...
	"sync"
	"time"
	spiderjobpb "spiderjob/lib/plugin/types"
...
var (
	ErrDependtJobs = errors.New("store: could not delete job with dependent jobs, delete childs first")
)
...
func NewStore() (*Store, error) {
	...
	db.CreateIndex("displayname", executionsPrefix + ":*", buntdb.IndexJSON("displayname"))
	db.CreateIndex("schedule", executionsPrefix + ":*", buntdb.IndexJSON("schedule"))
	db.CreateIndex("success_count", executionsPrefix + ":*", buntdb.IndexJSON("success_count"))
	db.CreateIndex("error_count", executionsPrefix + ":*", buntdb.IndexJSON("error_count"))
	db.CreateIndex("last_success", executionsPrefix + ":*", buntdb.IndexJSON("last_success"))
	db.CreateIndex("last_error", executionsPrefix + ":*", buntdb.IndexJSON("last_error"))
	db.CreateIndex("next", executionsPrefix + ":*", buntdb.IndexJSON("next"))
	if err != nil {
		return nil, err
	}
...
func (s *Store) setJobTxFunc(pbj *spiderjobpb.job) func(tx *buntdb.Tx) error {
...
func (s *Store) getJobTxFunc(name string, pbj *spiderjobpb.Job) func(tx *buntdb.Tx) error {
	return func(tx *buntdb.Tx) error {
...
func (*Store) setExecutionTxFunc(key string, pbe *spiderjobpb.Execution) func(tx *buntdb.Tx) error {
	return func(tx *buntdb.Tx) error {
